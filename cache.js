...
function analyzeAndStore(chat) {
  ...
  for (const msg of messages) {
    ...
    // Accumulate tokens for all message types (not just assistant)
    // Input tokens come from user messages, output tokens from assistant
...
function getCachedDeepAnalytics(opts = {}) {
  ...
  const log = (...args) => debug && console.log('[DeepAnalytics]', ...args);
  ...
  const hf = hiddenFolderFilter(opts, 'c.folder');
  log('Hidden folder filter:', hf);

  if (hf.sql) {
    sql += hf.sql;
    params.push(...hf.params);
    log('Applied hiddenFolderFilter SQL:', hf.sql);
  }

  if (opts.editor) {
    sql += ' AND c.source LIKE ?';
    params.push(`%${opts.editor}%`);
    log('Applied editor filter:', opts.editor);
  }

  if (opts.folder) {
    sql += ' AND c.folder = ?';
    params.push(opts.folder);
    log('Applied folder filter:', opts.folder);
  }

  if (opts.dateFrom) {
    sql += ' AND COALESCE(c.last_updated_at, c.created_at) >= ?';
    params.push(opts.dateFrom);
    log('Applied dateFrom filter:', opts.dateFrom);
  }

  if (opts.dateTo) {
    sql += ' AND COALESCE(c.last_updated_at, c.created_at) <= ?';
    params.push(opts.dateTo);
    log('Applied dateTo filter:', opts.dateTo);
  }
  ...
  log('Rows returned from DB:', rows.length);
  log('Raw rows:', rows);
  ...
  for (const r of rows) {
    ...
    // TOOL CALL PARSING
    try {
      log('Parsing tool_calls JSON:', r.tool_calls);

      const tools = JSON.parse(r.tool_calls);

      log('Parsed tools array:', tools);

      for (const t of tools) {
        toolFreq[t] = (toolFreq[t] || 0) + 1;
        totalToolCalls++;

        log(`Tool processed: ${t}`, {
          currentCount: toolFreq[t],
          totalToolCalls
        });
      }

    } catch (err) {
      log('Error parsing tool_calls JSON:', err);
    }

    // MODEL PARSING
    try {
      log('Parsing models JSON:', r.models);
      ...
      log('Error parsing models JSON:', err);
    }
  }

  log('---- FINISHED ROW PROCESSING ----');

  log('Tool frequency map:', toolFreq);
  log('Model frequency map:', modelFreq);

  // TOKEN ESTIMATION
  let tokensEstimated = false;

  log('Checking token estimation condition');

  if (
    totalInputTokens === 0 &&
    totalOutputTokens === 0 &&
    (totalUserChars > 0 || totalAssistantChars > 0)
  ) {
    log('No tokens found, estimating tokens from characters');
  ...
  log('Top tools calculated:', topTools);
  log('Top models calculated:', topModels);
